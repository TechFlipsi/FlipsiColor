...
namespace flipsicolor
{
    ...
    public:
        ...
        // GitHub API URL
        static constexpr const char* GITHUB_API_URL = "https://api.github.com/repos/TechFlipsi/FlipsiColor/releases";
    ...
    AutoUpdater::AutoUpdater(QObject* parent) : QObject(parent), m_impl(std::make_unique<Impl>())
    {
        m_impl->q = this;  // Pointer für emit-Callbacks

        m_impl->aktuelleVersion = QVersionNumber::fromString(QCoreApplication::applicationVersion());
    ...
    void AutoUpdater::Impl::pruefungStarten()
    {
        ...
        anfrage.setHeader(QNetworkRequest::UserAgentHeader, QString("FlipsiColor/%1").arg(aktuelleVersion.toString()));
        // Kein Content-Type auf GET-Request (hat keinen Body)

        QNetworkReply* antwort = networkManager.get(anfrage);

        QObject::connect(antwort, &QNetworkReply::finished, q, [this, antwort]() {
            antwort->deleteLater();
    ...
    void AutoUpdater::updateStarten()
    {
        ...
        if ( url.isEmpty() )
        {
            ...
            QDesktopServices::openUrl(QUrl("https://github.com/TechFlipsi/FlipsiColor/releases/latest"));
            return;
        }
        // Installer herunterladen
        m_impl->downloadPfad = QStandardPaths::writableLocation(QStandardPaths::TempLocation) + "/FlipsiColor-Update-" +
                               m_impl->neueVersionStr + ".exe";
        ...
        connect(antwort, &QNetworkReply::downloadProgress, this, [this](qint64 empfangen, qint64 gesamt) {
            double prozent = (gesamt > 0) ? (empfangen * 100.0 / gesamt) : 0.0;
            emit   downloadFortschritt(empfangen, gesamt, prozent);
        });

        connect(antwort, &QNetworkReply::finished, this, [this, antwort]() {
            antwort->deleteLater();
            if ( antwort->error() != QNetworkReply::NoError )
            {
    ...
    {
        return m_impl->updateVerfuegbar;
    }
    QString AutoUpdater::neueVersion() const
    {
        return m_impl->neueVersionStr;
    }
    QString AutoUpdater::aenderungen() const
    {
        return m_impl->aenderungenStr;
    }
    QString AutoUpdater::downloadUrl() const
    {
        return m_impl->downloadUrlStr;
    }
    qint64 AutoUpdater::downloadGroesse() const
    {
        return m_impl->downloadGroesseVal;
    }

    void AutoUpdater::setKanal(UpdateKanal k)
    {
        m_impl->kanal = k;
    }
    AutoUpdater::UpdateKanal AutoUpdater::kanal() const
    {
        return m_impl->kanal;
    }
