...
namespace flipsicolor
{
    ...
    void ModelManager::modellHerunterladen(ModellId id)
    {
        ...
        connect(reply, &QNetworkReply::downloadProgress, this, [this, id](qint64 empfangen, qint64 gesamt) {
    ...
    void ModelManager::modellLaden(ModellId id)
    {
        ...
        OrtEnv*    env    = nullptr;
        OrtStatus* status = ort()->CreateEnv(ORT_LOGGING_LEVEL_WARNING, "FlipsiColor", &env);
        if ( status )
        {
        ...
        status = ort()->CreateSession(
            env, reinterpret_cast<const wchar_t*>(vollerPfad.toStdWString().c_str()), opts, &session);
#else
        status = ort()->CreateSession(env, vollerPfad.toStdString().c_str(), opts, &session);
#endif
